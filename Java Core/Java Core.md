...
### [Работа с файловой системой](#работа-с-файловой-системой)<br>
...
13. **Signature-Version**: Версия подписи JAR-файла.
    ```makefile
...
4. **`t`**: Показать содержимое JAR-файла.
   ```
   jar tf myfile.jar
   ```
...
notifyAll(); // Пробуждает все потоки, которые ожидают на мониторинке объекта
...
```java
    ...
    \t - TAB (4 пробела)
    \" - перевод симола ["] к строковому формату
    \\ - вывод обычного [\]
```