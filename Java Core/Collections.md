...
Stack в Java — это класс, реализующий структуру данных "стек" (LIFO — Last In, First Out), где последний добавленный элемент извлекается первым. Stack является наследником класса Vector, поэтому он также наследует все методы Vector, но помимо этого предоставляет специфические методы для работы с "последовательностью

```java
  Stack<Integer> stack = new Stack<>();

  stack.push(item); // Добавляет элемент на вершину стека
  E item = stack.pop(); // Удаляет и возвращает элемент с вершины стека. Если стек пуст, метод генерирует EmptyStackException
  E item = stack.peek(); // Возвращает элемент на вершине стека, но не удаляет его. При попытке вызвать этот метод для пустого стека также выбрасывается EmptyStackException
  boolean empty = stack.isEmpty(); // Проверяет, пуст ли стек. Возвращает true, если в стеке нет элементов, и false — если есть
  int position = stack.search(o); // Ищет элемент в стеке и возвращает его позицию, считая от вершины. Если элемент найден, возвращается позиция (1 — вершина, 2 — следующий элемент и так далее); если элемент не найден, возвращается -1
```
